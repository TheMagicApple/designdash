<template>
  ...
  <div id="Inspector">
		...
		<input id="Height" class="slide" type="range" v-model="height" min="1" max="600">
	...
	export default {
		...
		created() {
			window.addEventListener('keydown', (e) => {
		  	...
		  	if(e.keyCode==37){
					this.boxes[this.boxInspector].left=(parseInt(this.boxes[this.boxInspector].left.substring(0,this.boxes[this.boxInspector].left.length-2))-5)+"px";
		  	}
		  	if(e.keyCode==38){
					this.boxes[this.boxInspector].top=(parseInt(this.boxes[this.boxInspector].top.substring(0,this.boxes[this.boxInspector].top.length-2))-5)+"px";
		  	}
		  	if(e.keyCode==39){
					this.boxes[this.boxInspector].left=(parseInt(this.boxes[this.boxInspector].left.substring(0,this.boxes[this.boxInspector].left.length-2))+5)+"px";
		  	}
